...
XXX 223 (ist eine Doppelung der 214) & 232 (enthält NAs) sind fehlerhaft --> Daten nochmal komplett neu ziehen!! XXX
...
knitr::kable(nof.table,
             caption = "N, M, SD, min & max number of fixations (micro-teaching unit)")
...
#### 2.2.2 Number of fixations (AOI students)
...
# N, M, SD, min & max number of fixations (AOI students)
nof.stud.table <-
  df_aoi_stud %>%
  group_by(Group) %>%
  summarise(
...
knitr::kable(nof.stud.table,
             caption = "N, M, SD, min & max number of fixations (AOI students)")
...
dur.table <- 
  ...
  summarise(
    N = n(),
    "M in ms" = round(mean(Average_duration_calc), digits = 2),
    "SD in ms" = round(sd(Average_duration_calc), digits = 2),
    "Min in ms" = round(min(Average_duration_calc), digits = 2),
    "Max in ms" = round(max(Average_duration_calc), digits = 2)
    )
...
# t-test for expertise differences
t.test(x = df_aoi_sum$Average_duration_calc[df_aoi_sum$Group == "Expert"],
       y = df_aoi_sum$Average_duration_calc[df_aoi_sum$Group == "Novice"],
       var.equal = TRUE)


# AVERAGE DURATION OF FIXATIONS on all AOIs
# effect size for expertise differences
d_aver_all <- CohenD(x = df_aoi_sum$Average_duration_calc[df_aoi_sum$Group == "Expert"],
                     y = df_aoi_sum$Average_duration_calc[df_aoi_sum$Group == "Novice"],
                    na.rm = TRUE)
round(d_aver_all, 2)
...
df_aoi_sum %>%
  group_by(Group) %>% 
  mutate(
    GRI = round(Average_duration_calc / Sum_number_fixation, digits = 2),
    )-> df_gri
...
#### 2.2.8 Gaze Relational Index (GRI; AOI students)
...
#### t-test & effect size "GRI" (AOI students)
...
#### 2.2.9 Gaze Relational Index (GRI; AOI disruptive person)
...
#### t-test & effect size "GRI" (AOI disruptive person)
...
# average duration & number fixation (micro-teaching unit)
df_aoi %>%
  filter(TOI == "Lesson") %>% 
  dplyr::select (
    Group,
