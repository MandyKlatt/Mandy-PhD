...
nof.table <-
  ...
  summarise(
    N = n(),
    "M" = round(mean(Number_fixation_sec_mtu), digits = 2),
    "SD" = round(sd(Number_fixation_sec_mtu), digits = 2),
    "Min" = round(min(Number_fixation_sec_mtu), digits = 2),
    "Max" = round(max(Number_fixation_sec_mtu), digits = 2)
  )
...
#### 2.2.2 Number of fixations per second (AOI students)
...
df_aoi %>%
  ...
  transmute(
    ...
    Stud_number_fixation_sec = round(Duration_of_interval_sec / Stud_number_fixation, digits = 2),
...
# N, M, SD, min & max number of fixations per seconds (AOI students)
nof.sec.stud.table <-
  df_aoi_stud %>%
  group_by(Group) %>%
  summarise(
    N = n(),
    "M" = round(mean(Stud_number_fixation_sec), digits = 2),
    "SD" = round(sd(Stud_number_fixation_sec), digits = 2),
    "Min" = round(min(Stud_number_fixation_sec), digits = 2),
    "Max" = round(max(Stud_number_fixation_sec), digits = 2)
  )

knitr::kable(nof.sec.stud.table,
             caption = "N, M, SD, min & max number of fixations per seconds (AOI students)")


# plotting number of fixations per seconds on AOI Students
df_aoi_stud %>%
  ggplot(mapping = aes(x = Group, y = Stud_number_fixation_sec)) +
  geom_boxplot(mapping = aes(fill = Group), outlier.shape = NA) +
  geom_point(
    size = 2,
    alpha = 0.7,
  ...
  theme_cowplot() +
  theme(legend.position = "none") -> plot_number_sec_stud

plot_number_sec_stud
...
# t-test for expertise differences
t.test(x = df_aoi_stud$Stud_number_fixation_sec[df_aoi_stud$Group == "Expert"],
       y = df_aoi_stud$Stud_number_fixation_sec[df_aoi_stud$Group == "Novice"],
       var.equal = TRUE)


# NUMBER OF FIXATIONS on AOI students
# effect size for expertise differences
d_number_stud <- CohenD(x = df_aoi_stud$Stud_number_fixation_sec[df_aoi_stud$Group == "Expert"],
                      y = df_aoi_stud$Stud_number_fixation_sec[df_aoi_stud$Group == "Novice"],
                      na.rm = TRUE)
...
df_aoi_sum %>%
  group_by(Group) %>% 
  mutate(
    GRI = round(Average_duration_mtu / Number_fixation_msec_mtu, digits = 2),
    )-> df_gri
