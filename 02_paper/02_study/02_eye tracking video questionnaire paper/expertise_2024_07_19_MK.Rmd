...
#### 2.2.1 Number of fixation in entire micro-teaching unit
...
# N, M, SD, min & max number of fixation (ent)
nof.table <-
  df_aoi_sum %>%
  group_by(Group) %>%
  summarise(
...
knitr::kable(nof.table,
             caption = "N, M, SD, min & max number of fixation in entire micro-teaching unit")
...
#### 2.2.2 Number of fixations on AOI "Students" during entire micro-teaching unit
...
knitr::kable(nof.stud.table,
             caption = "N, M, SD, min & max number of fixation on AOI students (entire micro-teaching unit)")
...
df_aoi_stud %>% 
  ...
  labs(x ="",
       y = "Number of fixation") + 
  scale_fill_brewer(palette  = "RdBu") +  
  ggtitle("Number of fixation on AOI\n`Students` (micro-teaching unit)") +
  theme_cowplot() +
...
#### 2.2.3 Average duration in entire micro-teaching unit
```{r dur_all, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

##################### AVERAGE DURATION ########################

# N, M, SD, min & max average duration (entire micro-teaching unit)
dur.table <-
  df_aoi_sum %>%
  group_by(Group) %>%
  summarise(
    N = n(),
    "M" = round(mean(Average_duration), digits = 2),
    "SD" = round(sd(Average_duration), digits = 2),
    "Min" = round(min(Average_duration), digits = 2),
    "Max" = round(max(Average_duration), digits = 2)
    )

knitr::kable(dur.table,
             caption = "N, M, SD, min & max average duration in entire micro-teaching unit")
...
```{r dur_ttest, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
#### 2.2.4 Average duration on AOI students
