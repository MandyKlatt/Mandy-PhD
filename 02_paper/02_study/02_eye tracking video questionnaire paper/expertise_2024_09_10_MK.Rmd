...
ggplot(df_aoi_sum, aes(x = "", y = Number_fixation_min_mtu)) + 
...
Q1 <- quantile(df_aoi_sum$Number_fixation_min_mtu, 0.25)
Q3 <- quantile(df_aoi_sum$Number_fixation_min_mtu, 0.75)
IQR <- Q3 - Q1
...
nrow(df_aoi_sum)  # Original data
nrow(df_no_outliers_mtu)  # After removing outliers
...
# Boxplot showing outliers
ggplot(df_aoi_stud, aes(x = "", y = Stud_number_fixation_min)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
...
# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_stud$Stud_number_fixation_min, 0.25)
Q3 <- quantile(df_aoi_stud$Stud_number_fixation_min, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_stud <- df_aoi_stud %>%
  filter(Stud_number_fixation_min >= LowerBound & Stud_number_fixation_min <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_stud)  # Original data
nrow(df_no_outliers_stud)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_stud <- df_aoi_stud %>%
  filter(Stud_number_fixation_min < LowerBound | Stud_number_fixation_min > UpperBound)
...
# t-test for expertise differences
t.test(
  x = df_no_outliers_stud$Stud_number_fixation_min[df_no_outliers_stud$Group == "Expert"],
  y = df_no_outliers_stud$Stud_number_fixation_min[df_no_outliers_stud$Group == "Novice"],
  var.equal = TRUE
)
...
# Boxplot showing outliers
ggplot(df_aoi_disrup, aes(x = "", y = Number_of_fixations.Disruptive_Person)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
...
# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_disrup$Number_of_fixations.Disruptive_Person, 0.25)
Q3 <- quantile(df_aoi_disrup$Number_of_fixations.Disruptive_Person, 0.75)
IQR <- Q3 - Q1
...
# Check the number of rows before and after filtering
nrow(df_aoi_disrup)  # Original data
nrow(df_no_outliers_disrup)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_disrup <- df_aoi_disrup %>%
  filter(Number_of_fixations.Disruptive_Person < LowerBound | Number_of_fixations.Disruptive_Person > UpperBound)
...
ggplot(df_aoi_sum, aes(x = "", y = Average_duration_mtu)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "Boxplot of Average duration of fixations per minute",
       y = "Average duration of fixations per minute") +
  theme_minimal()
...
# Boxplot showing outliers
ggplot(df_aoi_stud, aes(x = "", y = Average_duration_stud)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "Boxplot of average duration of fixations (AOI students)",
       y = "Average duration of fixations") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_stud$Average_duration_stud, 0.25)
Q3 <- quantile(df_aoi_stud$Average_duration_stud, 0.75)
IQR <- Q3 - Q1
...
# Check the number of rows before and after filtering
nrow(df_aoi_stud)  # Original data
nrow(df_no_outliers_stud_dur)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_stud_dur <- df_aoi_stud %>%
  filter(Average_duration_stud < LowerBound | Average_duration_stud > UpperBound)

# View the removed outliers
df_outliers_stud_dur


######## N, M, SD, min & max average duration of fixations on AOI students
dur.stud.table <-
  df_no_outliers_stud_dur %>%
  group_by(Group) %>%
  summarise(
    N = n(),
...
# Boxplot showing outliers
ggplot(df_aoi_disrup, aes(x = "", y = Average_duration_disrup)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
...
# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_disrup$Average_duration_disrup, 0.25)
Q3 <- quantile(df_aoi_disrup$Average_duration_disrup, 0.75)
IQR <- Q3 - Q1
...
# Create a dataframe with only the outliers
df_outliers_disrup_dur <- df_aoi_disrup %>%
  filter(Average_duration_disrup < LowerBound | Average_duration_disrup > UpperBound)

# View the removed outliers
df_outliers_disrup_dur


######## N, M, SD, min & max average duration of fixations in milliseconds (AOI disruptive person)"
dur.disrup.table <-
  df_no_outliers_disrup_dur %>%
  group_by(Group) %>%
  summarise(
    N = n(),
...
# Boxplot showing outliers
ggplot(df_gri, aes(x = "", y = GRI)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "Boxplot of the Gaze Relational Index",
       y = "Mean duration of fixations/\nmean number of fixations") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_gri$GRI, 0.25)
Q3 <- quantile(df_gri$GRI, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_gri <- df_gri %>%
  filter(GRI >= LowerBound & GRI <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_gri)  # Original data
nrow(df_no_outliers_gri)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_gri <- df_gri %>%
  filter(GRI < LowerBound | GRI > UpperBound)
...
df_sjt_mean$Group = ifelse(df_sjt_mean$Group < 200, "Novice","Expert")

#### ALL #### 
# t-test for expertise differences 
t.test(x = df_sjt_mean$All[df_sjt_mean$Group == "Expert"],
       y = df_sjt_mean$All[df_sjt_mean$Group == "Novice"],
       var.equal = TRUE)


# effect size for expertise differences
d_sjt_all <- CohenD(x = df_sjt_mean$All[df_sjt_mean$Group == "Expert"],
                    y = df_sjt_mean$All[df_sjt_mean$Group == "Novice"],
                    na.rm = TRUE
                    )
