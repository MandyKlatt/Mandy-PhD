...
# bookdown::word_document2 papaja::apa6_pdf
...
The term "professional competence" is the application of the concept to working life, particularly in highly complex and demanding professions, in which mastery of situations is especially dependent on the interplay of knowledge, skills, attitudes, and motivation." [cf. @epstein2002defining; cited in Kunter et al. 2013, p. 807]. Several studies could prove that this idea can be successfully transferred to the teaching profession [cf. Goodmann et al., 2008; @oser2006competence, Achtenhagen & Renold, 2006; Tannenbaum & Rosenfeld, 1994; cited in Kunter et al., 2013, p. 807]. Accordingly, a quality standard can be applied to assess the quality of a teacher. This quality is elementary in the school context because the teacher factor most influences student learning [@marzano2003classroom]. Thus, the effectiveness of education and the learning growth of individual students can be enhanced by the teacher. Research has shown that this component can even offset the influence of an ineffective school setting [@marzano2003classroom]. This effect places teachers at the center of ensuring effective teaching and learning and underscores the importance of professional competence in this career field.
...
However, lessons, and specifically classroom disruptions, are unpredictable as well as multifaceted and require a high level of expertise and competence on the part of the teacher to handle. This trained handling, as described above, can be summarized under the aspect of professional competence. It is expressed, among other things, in knowledge about classroom management [@grub2020process] and can be understood as a manifold toolbox of teachers to find their way in the challenging environment of the classroom. 

Classroom management significantly influences the ability to perceive and subsequently respond to classroom events and thus organize for disruption-free instruction [@wolff2015keeping]. @marzano2003classroom research created a foundational work in school and developmental research by manifesting that instructional strategy, curriculum design, and classroom management are significant roles in effective teaching. In this regard, research agrees that effective leadership and effective learning are closely related and classroom management can be considered a key component of teaching [@wolff2015keeping]. Therefore, it is often referred to as the central variable of successful teaching (Helmke & Helmke, 2015). Good classroom management is thus the prerequisite for learning to take place [@wolff2015keeping]. While the term suggests that it is mainly about rituals, rules, or measures that are meant to maintain order within the classroom, it is about much more than these two components. 
...
Based on the existing literature, we expected expert teachers to outperform novices by (H1) showing more fixations on relevant areas with (H2) shorter fixation duration and (H3) perceiving classroom disruptions faster [cf. @van2014first], (H4) feeling less troubled by disruptions and (H5) more confident in dealing with disruptions [@barth2017professionelle], and (H6) having higher scores in strategic knowledge of classroom management (Gold et al., 2016).
...
#### Videography: Coding scheme for expertise indicators
...
##### Teachers' Reaction to disruptions
...
```{r reaction_codingsystem, include=TRUE, echo=FALSE, results='asis'}
...
# papaja::apa_table(
#   x = df_react_table,
#   caption = "Self-developed coding scheme for the teacher's reaction",
#   align = c("m{8cm}", rep("m{2cm}", 5)),
#   col_spanners = list("NA"),
#   # note = "",
#   landscape = TRUE)
...
Redeanteil:
LO01_01	Redeanteil: Wieviel Prozent der gesamten Unterrichtslektion hat nach Ihrer Schätzung Ihr eigener Redeanteil eingenommen? ... %
LO01_02	Redeanteil: Wenn ich Fragen oder Aufgaben gestellt habe, habe ich den Lernenden folgende Zeit (in Sekunden) zum Nachdenken gegeben ... Sekunden
...
LV01_03	Natürliches Verhalten: Beim Unterrichten vor einer fiktiven Klasse habe ich mich so verhalten, wie ich es auch in der Schule tun würde.
...
```{r item_analysis_classroom_management, echo=FALSE, include=FALSE, results='asis'}
quest.data %>%
...
```{r item_analysis_balance, echo=FALSE, include=FALSE, results='asis'}
quest.data %>%
...
```{r item_analysis_presence, echo=FALSE, include=FALSE,results='asis'}
quest.data %>%
...
```{r item_analysis_behavior, echo=FALSE, include=FALSE,results='asis'}
quest.data %>%
...
#### Teachers' perception of disruption and confidence using Rating scales 
...
```{r rating_scales, include=TRUE, echo=FALSE, results='asis'}

# # read in data and combine multiple excel sheets into a single table 
# sri <- 
#   excel_sheets("data/Coding_SRI.xlsx") %>% 
#   map_df(~read_xlsx("data/Coding_SRI.xlsx",.)) %>% 
#   dplyr::select(Group, Event, `Disruption Factor`, `Confident Factor`) %>% # select relevant columns
#   dplyr::filter(!`Disruption Factor` == -100,
...
#   summarise("M" = round(mean(`Disruption Factor`), 
#                         digits = 2),
#             "SD" = round(sd(`Disruption Factor`),
#                          digits = 2),
#             "Min" = min(`Disruption Factor`),
#             "Max" = max(`Disruption Factor`))
# 
# # format and insert table in manuscript
# papaja::apa_table(
# sri.disrup.table,
#   caption = "Table Rating Scales Disruption Factor",
#   # note = "Write Note here",
#   escape = TRUE, # if TRUE special Latex characters are escaped; if this is turned to F captions cannot be rendered. I don't know why...
#   placement = "h", # position of table in page:  exact location (h), at the top (t), bottom (b)
#   font_size = "tiny" # options are tiny, scriptsize, footnotesize, small, normalsize (default), large, Large, LARGE, huge, Huge
...
#   group_by(Group, Event) %>%
#   mutate(Group = ifelse(Group < 200, "Novice","Expert")) %>%
#   summarise("M" = round(mean(`Confident Factor`), 
#                         digits = 2),
#             "SD" = round(sd(`Confident Factor`),
#                          digits = 2),
#             "Min" = min(`Confident Factor`),
#             "Max" = max(`Confident Factor`))
# 
# # format and insert table in manuscript
# papaja::apa_table(
# sri.confi.table,
#   caption = "Table Rating Scales Confident Factor",
#   # note = "Write Note here",
#   escape = TRUE, # if TRUE special Latex characters are escaped; if this is turned to F captions cannot be rendered. I don't know why...
#   placement = "h", # position of table in page:  exact location (h), at the top (t), bottom (b)
#   font_size = "tiny" # options are tiny, scriptsize, footnotesize, small, normalsize (default), large, Large, LARGE, huge, Huge
...
By *managing momentum* (structuring), @gold2015development understand the skillful control of teaching processes so that they can run smoothly and without delays, taking into account the students' level of understanding and attention [@kounin2006techniken, p. 101ff]. They also include the group focus specifically identified by Kounin [@kounin2006techniken, p. 117ff], i.e. mobilizing the whole class and demanding accountability [@gold2015development, p. 231]. 
...
```{r sjt, include=TRUE, echo=FALSE, results='asis'}

# read in data
df_sjt <- read_csv2(file = "data/SJT.csv") %>% 
  mutate(Group = ifelse(UI06_05 < 200, "Novice","Expert")) %>%
  dplyr::select(Group, 
                PK_S1_AL:PK_S14_R)
...
sjt.novice <- df_sjt %>% filter(Group == "Novice")
sjt.expert <- df_sjt %>% filter(Group == "Expert")
...
# creating a matrix
sjt.table.novice <- 
  matrix(data = NA, nrow = 3, ncol = 8)
  rownames(sjt.table.novice) <- c("AL", "ST", "R")
  colnames(sjt.table.novice) <- c("N Items","M","SD","Min","Max","Skewness","Kurtosis","alpha")
...
loop.data <- sjt.novice %>%
  dplyr::select(ends_with(match = i))

sjt.table.novice[i, "N Items"] <- length(loop.data)
...
loop.data %>%
  ...
  kurtosis() %>%
  round(., digits = 2) ->
sjt.table.novice[i, "Kurtosis"]

# calculating alpha
alpha <- loop.data %>%
  cronbach.alpha()

sjt.table.novice[i, "alpha"] <- round(alpha$alpha,2)

}

# rename columns
rownames(sjt.table.novice) <- c("Monitoring", "Managing Momentum", "Rules and routines")


# format and insert table in manuscript
papaja::apa_table(
sjt.table.novice,
  caption = "Scale analysis SJT novices",
  # note = "Write Note here",
  escape = TRUE, # if TRUE special Latex characters are escaped; if this is turned to F captions cannot be rendered. I don't know why...
  placement = "h", # position of table in page:  exact location (h), at the top (t), bottom (b)
  font_size = "tiny" # options are tiny, scriptsize, footnotesize, small, normalsize (default), large, Large, LARGE, huge, Huge
...
sjt.table.expert <- 
  matrix(data = NA, nrow = 3, ncol = 8)
  rownames(sjt.table.expert) <- c("AL", "ST", "R")
  colnames(sjt.table.expert) <- c("N Items","M","SD","Min","Max","Skewness","Kurtosis","alpha")
...
loop.data <- sjt.expert %>%
  dplyr::select(ends_with(match = i))

sjt.table.expert[i, "N Items"] <- length(loop.data)
...
loop.data %>%
  ...
  kurtosis() %>%
  round(., digits = 2) ->
sjt.table.expert[i, "Kurtosis"]

# calculating alpha
alpha <- loop.data %>%
  cronbach.alpha()

sjt.table.expert[i, "alpha"] <- round(alpha$alpha,2)

}

# rename columns
rownames(sjt.table.expert) <- c("Monitoring", "Managing Momentum", "Rules and routines")
...
Finally, the subjects answered a Situational Judgement Test of Strategic Knowledge of Classroom Management [SJT, @gold2015development] in the form of a questionnaire. Here they had to assess teaching scenarios and evaluate their behavior in response to them. The SJT was used to assess strategic knowledge about classroom management. The completion of the entire survey took 15 min on average.
