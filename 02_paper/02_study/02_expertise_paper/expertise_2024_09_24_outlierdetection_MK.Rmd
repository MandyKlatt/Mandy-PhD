...
df_aoi_sum_expert <- df_aoi_sum %>% 
  filter(Group == "Expert")

# Boxplot showing outliers for experts
ggplot(df_aoi_sum_expert, aes(x = "", y = Number_fixation_min_mtu)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of number of fixations per minute (micro-teaching unit)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_sum_expert$Number_fixation_min_mtu, 0.25)
Q3 <- quantile(df_aoi_sum_expert$Number_fixation_min_mtu, 0.75)
IQR <- Q3 - Q1
...
df_no_outliers_mtu_exp <- df_aoi_sum_expert %>%
  filter(Number_fixation_min_mtu >= LowerBound & Number_fixation_min_mtu <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_sum_expert)  # Original data
nrow(df_no_outliers_mtu_exp)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_mtu_exp <- df_aoi_sum_expert %>%
  filter(Number_fixation_min_mtu < LowerBound | Number_fixation_min_mtu > UpperBound)

# View the removed outliers
df_outliers_mtu_exp # ID 205


### NOVICES ### 

# filter only for novices
df_aoi_sum_novice <- df_aoi_sum %>% 
  filter(Group == "Novice")

# Boxplot showing outliers for novices
ggplot(df_aoi_sum_novice, aes(x = "", y = Number_fixation_min_mtu)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of number of fixations per minute (micro-teaching unit)",
       y = "Number of fixations per minute") +
  theme_minimal()
...
nof.table <-
  df_aoi_sum %>%
  filter(!Participant == "205") %>% # exclude outlier
  group_by(Group) %>%
  summarise(
    N = n(),
...
# create data frame without oulier 
df_aoi_sum_no_out <- df_aoi_sum %>% 
  filter(!Participant == "205")

# t-test for expertise differences
t.test(
  x = df_aoi_sum_no_out$Number_fixation_min_mtu[df_aoi_sum_no_out$Group == "Expert"],
  y = df_aoi_sum_no_out$Number_fixation_min_mtu[df_aoi_sum_no_out$Group == "Novice"],
  var.equal = TRUE
)
...
# filter only for experts
df_aoi_stud_expert <- df_aoi_stud %>% 
  filter(Group == "Expert")

# filter only for novices
df_aoi_stud_novice <- df_aoi_stud %>% 
  filter(Group == "Novice")
...
# Boxplot showing outliers for experts
ggplot(df_aoi_stud_expert, aes(x = "", y = Stud_number_fixation_min)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of number of fixations per minute (AOI students)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_stud_expert$Stud_number_fixation_min, 0.25)
Q3 <- quantile(df_aoi_stud_expert$Stud_number_fixation_min, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_stud_exp <- df_aoi_stud_expert %>%
  filter(Stud_number_fixation_min >= LowerBound & Stud_number_fixation_min <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_stud_expert)  # Original data
nrow(df_no_outliers_stud_exp)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_stud_exp <- df_aoi_stud_expert %>%
  filter(Stud_number_fixation_min < LowerBound | Stud_number_fixation_min > UpperBound)

# View the removed outliers
df_outliers_stud_exp


### NOVICES ### 

# Boxplot showing outliers for novices
ggplot(df_aoi_stud_novice, aes(x = "", y = Stud_number_fixation_min)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of number of fixations per minute (AOI students)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_stud_novice$Stud_number_fixation_min, 0.25)
Q3 <- quantile(df_aoi_stud_novice$Stud_number_fixation_min, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_stud_nov <- df_aoi_stud_novice %>%
  filter(Stud_number_fixation_min >= LowerBound & Stud_number_fixation_min <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_stud_novice)  # Original data
nrow(df_no_outliers_stud_nov)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_stud_nov <- df_aoi_stud_novice %>%
  filter(Stud_number_fixation_min < LowerBound | Stud_number_fixation_min > UpperBound)

# View the removed outliers
df_outliers_stud_nov


##### N, M, SD, min & max number of fixations per minute (AOI students)
nof.min.stud.table <-
  df_aoi_stud %>%
  filter(!Participant %in% c("205", "236", "241")) %>%
  group_by(Group) %>%
  summarise(
    N = n(),
...
# create data frame without oulier 
df_aoi_stud_no_out <- df_aoi_stud %>% 
  filter(!Participant %in% c("205", "236", "241")) 

# t-test for expertise differences
t.test(
  x = df_aoi_stud_no_out$Stud_number_fixation_min[df_aoi_stud_no_out$Group == "Expert"],
  y = df_aoi_stud_no_out$Stud_number_fixation_min[df_aoi_stud_no_out$Group == "Novice"],
  var.equal = TRUE
)
...
#### 2.2.3 Number of fixations (AOI others)
...
# filter only for experts
df_aoi_others_expert <- df_aoi_others %>% 
  filter(Group == "Expert")

# filter only for novices
df_aoi_others_novice <- df_aoi_others %>% 
  filter(Group == "Novice")
...
# Boxplot showing outliers for experts
ggplot(df_aoi_others_expert, aes(x = "", y = Others_number_fixation_min)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of number of fixations per minute (AOI others)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_others_expert$Others_number_fixation_min, 0.25)
Q3 <- quantile(df_aoi_others_expert$Others_number_fixation_min, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_others_exp <- df_aoi_others_expert %>%
  filter(Others_number_fixation_min >= LowerBound & Others_number_fixation_min <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_others_expert)  # Original data
nrow(df_no_outliers_others_exp)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_others_exp <- df_aoi_others_expert %>%
  filter(Others_number_fixation_min < LowerBound | Others_number_fixation_min > UpperBound)

# View the removed outliers
df_outliers_others_exp


### NOVICES ### 

# Boxplot showing outliers for novices
ggplot(df_aoi_others_novice, aes(x = "", y = Others_number_fixation_min)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of number of fixations per minute (AOI others)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_others_novice$Others_number_fixation_min, 0.25)
Q3 <- quantile(df_aoi_others_novice$Others_number_fixation_min, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_others_nov <- df_aoi_others_novice %>%
  filter(Others_number_fixation_min >= LowerBound & Others_number_fixation_min <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_others_novice)  # Original data
nrow(df_no_outliers_others_nov)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_others_nov <- df_aoi_others_novice %>%
  filter(Others_number_fixation_min < LowerBound | Others_number_fixation_min > UpperBound)

# View the removed outliers
df_outliers_others_nov
...
# Boxplot showing outliers for experts
ggplot(df_aoi_disrup_expert, aes(x = "", y = Number_of_fixations.Disruptive_Person)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of number of fixations per minute (AOI disruptive person)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_disrup_expert$Number_of_fixations.Disruptive_Person, 0.25)
Q3 <- quantile(df_aoi_disrup_expert$Number_of_fixations.Disruptive_Person, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_disrup_exp <- df_aoi_disrup_expert %>%
  filter(Number_of_fixations.Disruptive_Person >= LowerBound & Number_of_fixations.Disruptive_Person <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_disrup_expert)  # Original data
nrow(df_no_outliers_disrup_exp)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_disrup_exp <- df_aoi_disrup_expert %>%
  filter(Number_of_fixations.Disruptive_Person < LowerBound | Number_of_fixations.Disruptive_Person > UpperBound)

# View the removed outliers
df_outliers_disrup_exp


### NOVICES ### 

# Boxplot showing outliers for novices
ggplot(df_aoi_disrup_novice, aes(x = "", y = Number_of_fixations.Disruptive_Person)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of number of fixations per minute (AOI disruptive person)",
       y = "Number of fixations per minute") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_disrup_novice$Number_of_fixations.Disruptive_Person, 0.25)
Q3 <- quantile(df_aoi_disrup_novice$Number_of_fixations.Disruptive_Person, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_disrup_nov <- df_aoi_disrup_novice %>%
  filter(Number_of_fixations.Disruptive_Person >= LowerBound & Number_of_fixations.Disruptive_Person <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_disrup_novice)  # Original data
nrow(df_no_outliers_disrup_nov)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_disrup_nov <- df_aoi_disrup_novice %>%
  filter(Number_of_fixations.Disruptive_Person < LowerBound | Number_of_fixations.Disruptive_Person > UpperBound)

# View the removed outliers
df_outliers_disrup_nov


######## N, M, SD, min & max number of fixation on AOI "disruptive person"
nof.disrup.table <- df_aoi_disrup %>% 
  filter(!Participant %in% c("207", "226", "101", "108", "116")) %>% 
  group_by(Group) %>%
  summarise(
    N = n(),
...
# create data frame without oulier 
df_no_outliers_disrup <- df_aoi_disrup %>% 
  filter(!Participant %in% c("207", "226", "101", "108", "116"))

# t-test for expertise differences
t.test(x = df_no_outliers_disrup$Number_of_fixations.Disruptive_Person[df_no_outliers_disrup$Group == "Expert"],
       y = df_no_outliers_disrup$Number_of_fixations.Disruptive_Person[df_no_outliers_disrup$Group == "Novice"],
       var.equal = TRUE)

# NUMBER OF FIXATIONS 
# effect size for expertise differences
d_number_disrup <- CohenD(x = df_no_outliers_disrup$Number_of_fixations.Disruptive_Person[df_no_outliers_disrup$Group == "Expert"],
                        y = df_no_outliers_disrup$Number_of_fixations.Disruptive_Person[df_no_outliers_disrup$Group == "Novice"],
                        na.rm = TRUE)
...
# Boxplot showing outliers for experts
ggplot(df_aoi_sum_expert, aes(x = "", y = Average_duration_mtu)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of Average duration of fixations",
...
# Boxplot showing outliers for experts
ggplot(df_aoi_sum_novice, aes(x = "", y = Average_duration_mtu)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of Average duration of fixations",
       y = "Average duration of fixations in msec") +
  theme_minimal()
...
ggplot(df_aoi_stud_expert, aes(x = "", y = Average_duration_stud)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of average duration of fixations (AOI students)",
...
# Boxplot showing outliers
ggplot(df_aoi_stud_novice, aes(x = "", y = Average_duration_stud)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of average duration of fixations (AOI students)",
       y = "Average duration of fixations") +
  theme_minimal()

# Calculate the IQR and bounds
Q1 <- quantile(df_aoi_stud_novice$Average_duration_stud, 0.25)
Q3 <- quantile(df_aoi_stud_novice$Average_duration_stud, 0.75)
IQR <- Q3 - Q1
...
# Filter to remove outliers
df_no_outliers_stud_dur_nov <- df_aoi_stud_novice %>%
  filter(Average_duration_stud >= LowerBound & Average_duration_stud <= UpperBound)

# Check the number of rows before and after filtering
nrow(df_aoi_stud_novice)  # Original data
nrow(df_no_outliers_stud_dur_nov)  # After removing outliers

# Create a dataframe with only the outliers
df_outliers_dur_stud_nov <- df_aoi_stud_novice %>%
  filter(Average_duration_stud < LowerBound | Average_duration_stud > UpperBound)

# View the removed outliers
df_outliers_dur_stud_nov


######## N, M, SD, min & max average duration of fixations on AOI students
dur.stud.table <-
  df_aoi_stud %>%
  filter(!Participant %in% c("105", "133")) %>% 
  group_by(Group) %>%
  summarise(
    N = n(),
...
# create data frame without outlier
df_no_outliers_stud_dur <- df_aoi_stud %>%
  filter(!Participant %in% c("105", "133"))
  
# t-test for expertise differences
t.test(
  x = df_no_outliers_stud_dur$Average_duration_stud[df_no_outliers_stud_dur$Group == "Expert"],
  y = df_no_outliers_stud_dur$Average_duration_stud[df_no_outliers_stud_dur$Group == "Novice"],
  var.equal = TRUE
)
...
df_no_outliers_disrup_dur <- df_aoi_disrup %>%
  filter(!Participant %in% c("216", "105", "109", "133"))

# t-test for expertise differences
t.test(x = df_no_outliers_disrup_dur$Average_duration_disrup[df_no_outliers_disrup_dur$Group == "Expert"],
       y = df_no_outliers_disrup_dur$Average_duration_disrup[df_no_outliers_disrup_dur$Group == "Novice"],
       var.equal = TRUE)

# NUMBER OF FIXATIONS 
# effect size for expertise differences
d_dur_disrup <- CohenD(x = df_no_outliers_disrup_dur$Average_duration_disrup[df_no_outliers_disrup_dur$Group == "Expert"],
                        y = df_no_outliers_disrup_dur$Average_duration_disrup[df_no_outliers_disrup_dur$Group == "Novice"],
                        na.rm = TRUE)
...
GRI = Average duration of fixations (mtu) / Sum number of fixations (mtu)
...
ggplot(df_ttff_disrup_novice, aes(x = "", y = Disrup_time_fixation_sec)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "NOVICES: Boxplot of time to first fixation in seconds (AOI disruptive person)",
       y = "Time to first fixation in seconds") +
  theme_minimal()
...
#### t-test & effect size "Counts of the perceived ´disruptive person´")
```{r ttff_n_disrup_ttest, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
### 2.2 Rating Scales (Disruption Appraisal, Confidence Appraisal)
...
############## N, M, SD, min & max for disruption appraisal ############## 
sri.disrup.table <-
  sri %>%
  group_by(ID) %>%
  summarise(
...
# Boxplot showing outliers (MANAGING MOMENTUM)
...
ggplot(df_sjt_mean_expert, aes(x = "", y = Monitoring)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 19, outlier.size = 3) +
  labs(title = "EXPERTS: Boxplot of SJT (Monitoring)",
       y = "Score") +
  theme_minimal()
...
# create data frame without outliers
df_sjt_mean_mm_no_out <- df_sjt_mean %>% 
  filter(!Participant %in% c("105", "127"))

# t-test for expertise differences 
t.test(x = df_sjt_mean_mm_no_out$`Managing momentum`[df_sjt_mean_mm_no_out$Group == "Expert"],
       y = df_sjt_mean_mm_no_out$`Managing momentum`[df_sjt_mean_mm_no_out$Group == "Novice"],
       var.equal = TRUE
       )


# effect size for expertise differences
d_sjt_mm <- CohenD(x = df_sjt_mean_mm_no_out$`Managing momentum`[df_sjt_mean_mm_no_out$Group == "Expert"],
                   y = df_sjt_mean_mm_no_out$`Managing momentum`[df_sjt_mean_mm_no_out$Group == "Novice"],
                   na.rm = TRUE
                  )
...
# create data frame without outliers
df_sjt_mean_m_no_out <- df_sjt_mean %>% 
  filter(!Participant %in% c("121", "127", "137"))

# t-test for expertise differences 
t.test(x = df_sjt_mean_m_no_out$Monitoring[df_sjt_mean_m_no_out$Group == "Expert"],
       y = df_sjt_mean_m_no_out$Monitoring[df_sjt_mean_m_no_out$Group == "Novice"],
       var.equal = TRUE
       )


# effect size for expertise differences
d_sjt_moni <- CohenD(x = df_sjt_mean_m_no_out$Monitoring[df_sjt_mean_m_no_out$Group == "Expert"],
                     y = df_sjt_mean_m_no_out$Monitoring[df_sjt_mean_m_no_out$Group == "Novice"],
                     na.rm = TRUE
                    )
...
# create data frame without outliers
df_sjt_mean_r_no_out <- df_sjt_mean %>% 
  filter(!Participant %in% c("127"))

# t-test for expertise differences 
t.test(x = df_sjt_mean_r_no_out$`Rules and routines`[df_sjt_mean_r_no_out$Group == "Expert"],
       y = df_sjt_mean_r_no_out$`Rules and routines`[df_sjt_mean_r_no_out$Group == "Novice"],
       var.equal = TRUE
      )


# effect size for expertise differences
d_sjt_rr <- CohenD(x = df_sjt_mean_r_no_out$`Rules and routines`[df_sjt_mean_r_no_out$Group == "Expert"],
                   y = df_sjt_mean_r_no_out$`Rules and routines`[df_sjt_mean_r_no_out$Group == "Novice"],
                   na.rm = TRUE
                  )
...
#                 ) -> df_aoi_disrup
# 
# # time to first fixation (AOI disruptive person)
# df_aoi %>%
#   filter(
#     TOI %in% c(
