...
# Post-hoc comparisons for Disruption_Category
post_hoc_disruption <- emmeans(anova_afex, pairwise ~ Disruption_Category)

# Extract pairwise contrasts
post_hoc_results <- as.data.frame(post_hoc_disruption$contrasts)

# Clean up category names in the contrast column
post_hoc_results$contrast <- gsub("_", " ", post_hoc_results$contrast)
post_hoc_results$contrast <- gsub("\\.", " ", post_hoc_results$contrast)

# Calculate Cohen's d for each pairwise comparison using pooled SD from emmeans
post_hoc_results <- post_hoc_results %>%
  mutate(
    pooled_sd = sqrt((SE^2 * 2)), # Approximation for pooled SD from SE
    d = round(estimate / pooled_sd, 2), # Calculate Cohen's d
    p.value = ifelse(
      p.value < 0.05, "< .05", round(p.value, 3)
    ),
    t = round(t.ratio, 2),
    df = round(df, 2),
  ...
  select(contrast, Estimate, SE, df, t, p.value, d) # Include Cohen's d

# Display APA-styled post-hoc table
kable(
  post_hoc_results,
...
plot_overall <- ggplot(individual_summary, aes(x = Group, y = mean_duration, fill = Group)) +
  ...
  ggtitle("Overall Average Duration per Participant\n(AOI Disruptive Person)") +
  scale_fill_brewer(palette = "RdBu") +
  theme_cowplot() +
  ylim(0, y_max_overall) +
  theme(legend.position = "none") +
...
# Plot by category
plot_category <- disruption_summary %>%
  ggplot(aes(x = Group, y = mean_duration_ms, fill = Group)) +
  geom_boxplot(outlier.shape = NA, width = 0.5) +
...
# Perform post-hoc pairwise comparisons for Group
post_hoc_group <- emmeans::emmeans(anova_afex, pairwise ~ Group)

# Extract post-hoc pairwise contrasts for Group
post_hoc_results_group <- as.data.frame(post_hoc_group$contrasts) %>%
  mutate(
    ...
    SE = round(SE, 2),
    t = round(t.ratio, 2),
    p.value = ifelse(p.value < 0.05, "< .05", round(p.value, 3)),
    Cohen_d = round(estimate / sqrt(SE^2 * 2), 2) # Approximate Cohen's d
  ) %>%
  select(Comparison, Estimate, SE, df, t, p.value, Cohen_d) # Keep relevant columns

# Perform post-hoc pairwise comparisons for Disruption_Category
post_hoc_disruption <- emmeans::emmeans(anova_afex, pairwise ~ Disruption_Category)
...
# Display the post-hoc test results for Disruption_Category
knitr::kable(
...
normality_tests <- df_ttff_disrup %>%
  group_by(Group) %>%
  summarise(
    Shapiro_Wilk_p = round(shapiro.test(Disrup_time_fixation_sec)$p.value, 3),
    .groups = "drop"
  )
...
cor_matrix <- cor(df_correlation, use = "complete.obs")

# Visualize the correlation matrix
corrplot(cor_matrix, method = "circle", type = "lower", order = "hclust", tl.col = "black", tl.srt = 45)


# 4. Correlation Analysis with Experts ---------------------------------------------

# Filter data for Experts (Participants with ID > 200)
df_experts <- df_merge %>%
  filter(Participant > 200)  # Only include Experts (ID > 200)

# Select relevant variables for correlation analysis with Teaching Experience
df_correlation_experts <- df_experts %>%
  select(
    `Teaching Experience`, 
    Number_fixation_min_mtu, 
    Average_duration_mtu, 
...
# Check the column types to make sure they are numeric
sapply(df_correlation_experts, class)

# Compute the correlation matrix
cor_matrix_experts <- cor(df_correlation_experts, use = "complete.obs")

# Visualize the correlation matrix
corrplot(cor_matrix_experts, method = "circle", type = "lower", order = "hclust", tl.col = "black", tl.srt = 45)


# 5. Correlation Analysis with Novices ---------------------------------------------

# Filter data for Novices (Participants with ID < 200)
df_novices <- df_merge %>%
  filter(Participant < 200)  # Only include Novices (ID < 200)

# Select relevant variables for correlation analysis without Teaching Experience
df_correlation_novices <- df_novices %>%
  select(
    Number_fixation_min_mtu, 
    Average_duration_mtu, 
    GRI_mtu, 
...
# Ensure all selected columns are numeric and handle non-numeric entries
df_correlation_novices_clean <- df_correlation_novices %>%
  mutate(across(everything(), as.numeric))  # Convert all columns to numeric

# Remove rows with any NA or problematic values
df_correlation_novices_clean <- df_correlation_novices_clean %>%
  drop_na()  # Remove rows with NA values

# Check if all columns are numeric after conversion
sapply(df_correlation_novices_clean, class)

# Compute the correlation matrix for Novices
cor_matrix_novices <- cor(df_correlation_novices_clean, use = "complete.obs")

# Visualize the correlation matrix for Novices
corrplot(cor_matrix_novices, method = "circle", type = "lower", order = "hclust", tl.col = "black", tl.srt = 45)
```
