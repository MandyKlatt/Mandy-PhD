...
quest_plot <- df_quest_long %>%
  mutate(
    ...
    `Scales Self-evaluation` = factor(`Scales Self-evaluation`, levels = c("Classroom Management + 3 Items", "Presence", "Overall"))  # Reorder scales
  ) %>%
...
t_test_cm <- t_test_stats_long(df_quest_long, "Group", "Classroom Management + 3 Items", "Classroom Management + 3 Items")
...
omega_cm <- calculate_omega_total(df_cm, "Classroom Management + 3 Items")
...
omega_results <- tibble(
  Facet = c("Overall Evaluation", "Classroom Management + 3 Items", "Presence"),
...
disruption_summary <- df_aoi_disrup %>%
  ...
  summarise(
    total_duration_sec = round(sum(Duration_of_interval_sec, na.rm = TRUE), 2),  # Total time in seconds
    fixation_count = round(sum(`Number_of_fixations.Disruptive_Person`, na.rm = TRUE), 2),  # Total fixations
    Disrup_fixations_per_second = round(fixation_count / total_duration_sec, 2),  # Number of Fixation per second
...
df_aoi_disrup <- disruption_summary %>%
  group_by(Group, Participant) %>%
  summarise(
    Disrup_fixations_per_second = round(mean(Disrup_fixations_per_second, na.rm = TRUE), 2),  # Mean Number of Fixation per participant
