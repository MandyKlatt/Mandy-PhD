...
# Gaze Behavior Across Areas of Interest 
```{r nof__dur_percent, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
# Step 3: Calculate total fixation duration per participant, excluding "Disruptive Student"
...
grouped_aoi_durations_percentages <- aoi_durations_percentages %>%
  ...
  mutate(AOI = recode(AOI,
                      ...
                      "Mean_AOI_Disruptive_Person_pct" = "Disruptive Student",
...
# Step 6: Restructure data for grouping and calculate 95% CI for fixation durations
grouped_aoi_durations_ci <- aoi_durations_percentages %>%
  pivot_longer(
    cols = starts_with("AOI"),
    names_to = "AOI",
    values_to = "Duration_Percentage"
  ) %>%
  mutate(AOI = recode(
    AOI,
    "AOI_Students_pct" = "Students",
    "AOI_Disruptive_Person_pct" = "Disruptive Student",
    "AOI_Teacher_Material_pct" = "Teacher Material",
    "AOI_Student_Desk_pct" = "Student Desk",
    "AOI_Classroom_Others_pct" = "Classroom/Others"
  )) %>%
  group_by(Group, AOI) %>%
  summarise(
    Mean_Duration_Percentage = mean(Duration_Percentage, na.rm = TRUE),
    CI_Lower = Mean_Duration_Percentage - qt(0.975, df = n() - 1) * sd(Duration_Percentage, na.rm = TRUE) / sqrt(n()),
    CI_Upper = Mean_Duration_Percentage + qt(0.975, df = n() - 1) * sd(Duration_Percentage, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )

# Step 7: Plot with 95% confidence intervals
percentage_duration_fixation <- ggplot(
  grouped_aoi_durations_ci %>%
    mutate(Group = recode(Group, "Expert" = "Experienced", "Novice" = "Inexperienced")), # Rename groups
  aes(
    x = reorder(AOI, -Mean_Duration_Percentage),
    y = Mean_Duration_Percentage,
    fill = Group
  )
) +
  ...
  geom_text(
    aes(label = paste0(round(Mean_Duration_Percentage, 2), "%")),
    position = position_dodge(width = 0.8),
    vjust = -2.5,  # Adjusted to place the labels above the bars
    size = 3
  ) +
  ylim(0, 60) + 
  labs(
    title = "Average Fixation Duration Percentage by AOI and Group",
    x = "AOI",
...
percentage_duration_fixation

ggsave(
  filename = "plots/percentage_duration_fixation_with_ci.svg",
  width = 20,
  height = 12,
  units = "cm"
)
...
# Map AOI names to clean versions for readability in the final results_duration table
results_duration$AOI <- recode(results_duration$AOI,
                      "AOI_Students" = "Students",
                      "AOI_Disruptive_Person" = "Disruptive Student",
...
# Step 3: Calculate total fixations per participant, excluding "Disruptive Student"
# Extract relevant column names excluding "Disruptive Student"
relevant_columns <- unlist(aoi_columns[c("AOI_Students", "AOI_Teacher_Material", "AOI_Student_Desk", "AOI_Classroom_Others")])
...
# Step 6: Restructure data for grouping and calculate 95% CI
grouped_aoi_percentages_ci <- aoi_percentages %>%
  pivot_longer(
    cols = starts_with("AOI"),
    names_to = "AOI",
  ...
  summarise(
    ...
    CI_Lower = Mean_Percentage - qt(0.975, df = n() - 1) * sd(Percentage, na.rm = TRUE) / sqrt(n()),
    CI_Upper = Mean_Percentage + qt(0.975, df = n() - 1) * sd(Percentage, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )

# Step 7: Plot with 95% confidence intervals
percentage_number_fixation <- ggplot(
  grouped_aoi_percentages_ci %>% 
    mutate(Group = recode(Group, 
                          "Expert" = "Experienced", 
                          "Novice" = "Inexperienced")),  # Rename groups
  aes(
    x = reorder(AOI, -Mean_Percentage),
    y = Mean_Percentage,
    fill = Group
  )
) +
  ...
  geom_text(
    aes(label = paste0(round(Mean_Percentage, 2), "%")),
    position = position_dodge(width = 0.8),
    vjust = -2.5,  # Adjusted to place the labels above the bars
    size = 3
  ) +
  labs(title = "Average Fixation Number Percentage by AOI and Group", 
       x = "AOI", 
...
ggsave(filename = "plots/percentage_number_fixation_with_ci.svg",
       width = 20,
       height = 12,
       units = "cm")
...
for (column_name in aoi_list) {
  ...
  if (column_name %in% names(aoi_percentages)) {
    ...
    if (length(expert_data) > 0 && length(novice_data) > 0) {
      ...
      # Format the results_number for the current AOI and add to the results_number data frame
      results_number <- rbind(
        results_number,
        data.frame(
...
results_number$AOI <- aoi_name_mapping[results_number$AOI]

# Display the combined results_number table in APA style without row names
knitr::kable(results_number, caption = "t-Test and Effect Size for Fixation Percentage across AOIs (Experts vs. Novices)", row.names = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)

```


#### Number of fixations per minute and fixation duration in ms (micro-teaching unit)
```{r nof_dur_all, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
# nof_stats <- df_aoi_sum %>%
#   group_by(Group) %>%
#   summarise(
#     N = n(),
#     M = round(mean(Number_fixation_min_mtu, na.rm = TRUE), 2),
#     SD = round(sd(Number_fixation_min_mtu, na.rm = TRUE), 2),
#     Min = round(min(Number_fixation_min_mtu, na.rm = TRUE), 2),
#     Max = round(max(Number_fixation_min_mtu, na.rm = TRUE), 2)
#   )
# 
# 
# # Display Table
# knitr::kable(nof_stats, caption = "N, M, SD, Min & Max for Number of Fixations per Minute") %>%
#   kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
# 
# # t-Test and Cohen's d
# t_test_mtu <- t.test(Number_fixation_min_mtu ~ Group, data = df_aoi_sum, var.equal = TRUE)
# cohen_d_mtu <- cohen.d(Number_fixation_min_mtu ~ Group, data = df_aoi_sum)
# 
# # APA Table
# t_test_result_mtu <- data.frame(
#   "Group Comparison" = "Experienced vs. Inexperienced",
...
#   "Cohen's d" = round(cohen_d_mtu$estimate, 2),
#   "95% CI (d)" = paste0("[", round(cohen_d_mtu$conf.int[1], 2), ", ", round(cohen_d_mtu$conf.int[2], 2), "]")
# )
# 
# knitr::kable(t_test_result_mtu, caption = "t-Test and Effect Size for Number of Fixations per Minute") %>%
#   kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
# 
# # Ensure cohen_d_mtu is numeric
# cohen_d_mtu <- round(as.numeric(cohen.d(Number_fixation_min_mtu ~ Group, data = df_aoi_sum)$estimate), 2)
# 
# # Plot: Number of Fixations per Minute
# y_max_mtu <- max(df_aoi_sum$Number_fixation_min_mtu) * 1.5
# 
# plot_number_fixation_group <- df_aoi_sum %>%
#   ggplot(aes(x = Group, y = Number_fixation_min_mtu, fill = Group)) +
...
#   scale_fill_brewer(palette = "RdBu") +
#   ylim(0, y_max_mtu) +
#   labs(title = "Number of Fixations per Minute\n(Micro-Teaching Unit)", x = "", y = "Fixations per Minute") +
#   theme_cowplot() +
#   theme(legend.position = "none") +
#   annotate("text", x = 1, y = 3,
#            label = paste0("italic(M) == ", round(nof_stats$M[1], 2), " * ',' ~ italic(SD) == ", round(nof_stats$SD[1], 2)),
#            hjust = 0.5, size = 4, parse = TRUE) +
#   annotate("text", x = 2, y = 3,
#            label = paste0("italic(M) == ", round(nof_stats$M[2], 2), " * ',' ~ italic(SD) == ", round(nof_stats$SD[2], 2)),
#            hjust = 0.5, size = 4, parse = TRUE) +
#   annotate("text", x = 1.5, y = y_max_mtu * 0.95,
#            label = paste0("italic(p) == ", round(t_test_mtu$p.value, 2), " * ',' ~ italic(d) == ", cohen_d_mtu),
#            hjust = 0.5, size = 4, fontface = "italic", parse = TRUE)
# 
# plot_number_fixation_group
# 
# ggsave(filename = "plots/plot_number_fixation_group.svg",
#        width = 20,
#        height = 12,
#        units = "cm")
# 
...
# fixation_stats <- df_aoi_sum %>%
#   group_by(Group) %>%
#   summarise(
#     N = n(),
#     M = round(mean(Avg_fixation_duration, na.rm = TRUE), 2),
#     SD = round(sd(Avg_fixation_duration, na.rm = TRUE), 2),
#     Min = round(min(Avg_fixation_duration, na.rm = TRUE), 2),
#     Max = round(max(Avg_fixation_duration, na.rm = TRUE), 2)
#   )
# 
# # Display Table
# knitr::kable(fixation_stats, caption = "N, M, SD, Min & Max for Average Fixation Duration") %>%
#   kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
# 
# # t-Test and Cohen's d
# t_test_fixation <- t.test(Avg_fixation_duration ~ Group, data = df_aoi_sum, var.equal = TRUE)
# cohen_d_fixation <- cohen.d(Avg_fixation_duration ~ Group, data = df_aoi_sum)
# 
# # APA Table
# t_test_result_fixation <- data.frame(
#   "Group Comparison" = "Experienced vs. Inexperienced",
#   "t-value" = round(t_test_fixation$statistic, 2),
#   "df" = t_test_fixation$parameter,
#   "p-value" = format.pval(t_test_fixation$p.value, digits = 2, eps = .05),
#   "Mean Difference" = round(t_test_fixation$estimate[1] - t_test_fixation$estimate[2], 2),
#   "Cohen's d" = round(cohen_d_fixation$estimate, 2),
#   "95% CI (d)" = paste0("[", round(cohen_d_fixation$conf.int[1], 2), ", ", round(cohen_d_fixation$conf.int[2], 2), "]")
# )
# 
# knitr::kable(t_test_result_fixation, caption = "t-Test and Effect Size for Average Fixation Duration") %>%
#   kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
# 
# # Ensure cohen_d_fixation is numeric
# cohen_d_fixation <- round(cohen.d(Avg_fixation_duration ~ Group, data = df_aoi_sum)$estimate, 2)
# 
# # Plot: Average Fixation Duration
# plot_avg_fixation_group <- df_aoi_sum %>%
#   ggplot(aes(x = Group, y = Avg_fixation_duration, fill = Group)) +
#   geom_boxplot(outlier.shape = 16, outlier.size = 2, width = 0.5) +
#   geom_jitter(width = 0.15, height = 0.1, alpha = 0.5, color = "black", size = 1.5) +
#   stat_summary(fun = mean, geom = "point", shape = 4, size = 5, color = "blue", stroke = 1.2) +
#   scale_fill_brewer(palette = "RdBu") +
#   labs(title = "Average Fixation Duration\n(Micro-Teaching Unit)", x = "", y = "Fixation Duration (ms)") +
#   theme_cowplot() +
#   theme(legend.position = "none") +
#   # Add M and SD values below the x-axis for each group
#   annotate("text", x = 1, y = -50,  # Adjust y-coordinate to place below the x-axis
#            label = paste0("italic(M) == ", round(fixation_stats$M[1], 2), " * ',' ~ italic(SD) == ", round(fixation_stats$SD[1], 2)),
#            hjust = 0.5, size = 4, parse = TRUE) +
#   annotate("text", x = 2, y = -50,  # Adjust y-coordinate to place below the x-axis
#            label = paste0("italic(M) == ", round(fixation_stats$M[2], 2), " * ',' ~ italic(SD) == ", round(fixation_stats$SD[2], 2)),
#            hjust = 0.5, size = 4, parse = TRUE) +
#   # Add p-value and d-value above the plot
#   annotate("text", x = 1.5, y = max(df_aoi_sum$Avg_fixation_duration) * 1.1,
#            label = paste0("italic(p) == ", round(t_test_fixation$p.value, 2), " * ',' ~ italic(d) == ", cohen_d_fixation),
#            hjust = 0.5, size = 4, fontface = "italic", parse = TRUE)
# 
# plot_avg_fixation_group
# 
# ggsave(filename = "plots/plot_avg_fixation_group.svg",
#        width = 20,
#        height = 12,
#        units = "cm")
# 
...
# combined_plot <- plot_number_fixation_group + plot_avg_fixation_group +
...
# ggsave(
#   filename = "plots/combined_results_plot.svg",
#   plot = combined_plot,
#   width = 25,
#   height = 12,
#   units = "cm"
...
```{r nof_dur_students, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
#### Number of Fixations per Minute and Average Fixation Duration (AOI Students)
...
#### Number of fixations per seconds (AOI Disruptive Student)
```{r nof_disrup, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

##################### NUMBER OF FIXATIONS ON AOI Disruptive Student ########################
...
kable(
  overall_stats,
  caption = "Descriptive Statistics (N, Mean, SD, Min, Max) for Number of Fixations by Group (AOI Disruptive Student)",
...
kable(
  category_stats,
  caption = "Descriptive Statistics (N, Mean, SD, Min, Max) for Number of Fixations by Group and Category (AOI Disruptive Student)",
  col.names = c("Group", "Category", "N", "M", "SD", "Min", "Max"),
  row.names = FALSE
) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)

##################### FIRST PLOT: Number of Fixations AOI Disruptive Student ########################
...
##################### SECOND PLOT: NUMBER OF FIXATIONS BY CATEGORY (AOI Disruptive Student) ########################
...
kable(
  overall_effect,
  caption = "t-Test and Effect Size for Number of Fixations (AOI Disruptive Student)",
...
##################### AVERAGE DURATION ON AOI Disruptive Student ########################
##################### DATA PREPARATION ########################
...
#### Time to first fixation in seconds (AOI Disruptive Student)
...
### Self-evaluations of disruption prevention and management competencies (5 classroom management items + 3 self-developed, 8 presence items)
...
# Create APA-Conform Table for Overall Evaluation
overall_summary <- df_quest_long %>%
  filter(`Scales Self-evaluation` == "Overall") %>%
  group_by(Group) %>%
  summarise(
    N = n(),
...
# Display the table in APA format
kable(overall_summary, caption = "Descriptive Statistics for Overall Evaluation") %>%
...
# # Create Dataset for Plotting with Overall Score Only -------------------------
# df_quest_long <- df_quest %>%
#   rowwise() %>%
#   transmute(
#     Group = Group,
#     `Overall` = round(mean(c(CM_01, CM_02, CM_03, CM_04, CM_05, CM_06, CM_07, CM_08, P_01, P_02, P_03, P_04, P_05, P_06, P_07, P_08), na.rm = TRUE), 2)  # Overall score only
#   ) %>%
#   ungroup()
# 
# # Plot Overall Mean Scores by Group ----------------------------------------------
# quest_plot <- df_quest_long %>%
#   mutate(
#     Group = factor(Group, levels = c("Novice", "Expert"))
#   ) %>%
#   ggplot(aes(x = Group, y = Overall, fill = Group)) +
#   geom_boxplot(outlier.shape = NA) +
#   geom_point(
#     size = 1,
#     alpha = 0.4,
#     position = position_jitter(seed = 1, width = 0.1, height = 0.1)
#   ) +
#   labs(x = "", y = "Overall Mean Score") +
#   ylim(0, 4) + 
#   scale_fill_brewer(palette = "RdBu") +
#   theme_cowplot() +
#   ggtitle("Overall Self-Evaluation Scores") +
#   theme(
#     plot.title = element_text(size = 25, face = "bold"),
#     legend.title = element_text(size = 20),
#     legend.text = element_text(size = 15),
...
# Read the dataset
sri <- read_excel("data/Coding_SRI.xlsx") %>%
  filter(
  ...
  mutate(
    ...
    ID = as.factor(ID)  # Convert ID to factor if needed
  )
...
# Ensure no duplicates (keeping one row per participant)
sri_clean <- sri %>%
  group_by(ID) %>%
  summarise(
    Group = first(Group),
    disruption_appraisal = mean(disruption_appraisal, na.rm = TRUE),
    confidence_appraisal = mean(confidence_appraisal, na.rm = TRUE)
  ) %>%
  ungroup()

# Check the number of unique participants
print(nrow(sri_clean))  # Should be 82

# t-Test for Disruption Appraisal
t_test_disrup <- t.test(disruption_appraisal ~ Group, data = sri_clean, var.equal = TRUE)
cohen_d_disrup <- cohen.d(disruption_appraisal ~ Group, data = sri_clean)

# t-Test for Confidence Appraisal
t_test_confi <- t.test(confidence_appraisal ~ Group, data = sri_clean, var.equal = TRUE)
cohen_d_confi <- cohen.d(confidence_appraisal ~ Group, data = sri_clean)

# Create APA-Formatted Table
t_test_results <- data.frame(
  "Group Comparison" = c("Experienced vs. Inexperienced", "Experienced vs. Inexperienced"),
  "Variable" = c("Disruption Appraisal", "Confidence Appraisal"),
  "t-value" = round(c(t_test_disrup$statistic, t_test_confi$statistic), 2),
  "df" = c(t_test_disrup$parameter, t_test_confi$parameter),
  "p-value" = format.pval(c(t_test_disrup$p.value, t_test_confi$p.value), digits = 3, eps = .001),
  "Mean Difference" = round(c(
    diff(t_test_disrup$estimate), 
    diff(t_test_confi$estimate)
  ), 2),
  "Cohen's d" = round(c(cohen_d_disrup$estimate, cohen_d_confi$estimate), 2),
  "95% CI (d)" = c(
    paste0("[", round(cohen_d_disrup$conf.int[1], 2), ", ", round(cohen_d_disrup$conf.int[2], 2), "]"),
    paste0("[", round(cohen_d_confi$conf.int[1], 2), ", ", round(cohen_d_confi$conf.int[2], 2), "]")
  )
)

# Print APA-formatted Table
knitr::kable(t_test_results, caption = "t-Test and Effect Sizes for Disruption and Confidence Ratings") %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
...
### Correlation table for all participants
