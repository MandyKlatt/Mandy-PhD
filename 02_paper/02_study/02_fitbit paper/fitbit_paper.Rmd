...
Prospective teachers experience the first five years as a particular professional challenge, as they are associated with a high workload (e.g., class size, poor student behavior) and resulting stress in the teaching profession [@struyven2014teachers].

Psychosomatic stress symptoms, such as increasing heart rate, result from an interaction of situational stressors and person-specific available resources ["transactional stress model"; @lazarus1987transactional; @obbarius2021modified]. If teachers are exposed to a stressful teaching-learning environment for a long time and do not have the sufficient resources and coping strategies, this can lead to burnout. 
...
In addition to self-reported indicators of stress (e.g., emotional exhaustion), psychophysiological indicators (e.g., heart rate) are becoming increasingly important in research. Heart rate as the number of heart contractions per minute [@Löllgen2015] is an important indicator of physical or emotional stress. Increased workload is associated with increased heart rate [@sachs2014physiologische]. Arguably, physiological data provide a more objective and non-reactive measure of stress in the classroom (Runge et al., 2020). However, in an educational context, this requires the use of low-cost, non-invasive instruments. Fitness trackers worn on the wrist could fill this research gap [@ferguson2015validity]. 
...
demo.data$ID = ifelse(demo.data$ID < 200, "Novice","Expert")
...
The sample consisted of *N* = `r length(demo.data$ID)` participants with *n* = `r length(demo.data$ID[demo.data$ID == "Expert"])` expert teachers and *n* = `r length(demo.data$ID[demo.data$ID == "Novice"])` novice teachers. 
...
The expert teachers (`r demo.data %>% filter(Gender == 1 & ID == "Expert") %>% pull(Gender) %>% length()` women; `r demo.table %>% filter(ID == "Expert") %>% pull("Gender female in percent")`%) had a mean age of `r demo.data %>% filter(ID == "Expert") %>% pull(Age) %>% mean() %>% round(., digits = 1)` years (*SD* = `r demo.table %>% filter(ID == "Expert") %>% pull("SD Age in years") %>% round(., digits = 1)`; range: `r demo.table %>%  filter(ID == "Expert") %>% pull("Min Age in years")`-`r demo.table %>%  filter(ID == "Expert") %>% pull("Max Age in years")`) and an average teaching experience of `r exp.table %>% filter(ID == "Expert") %>% pull("M Exp.") %>% mean() %>% round(., digits = 1)` years (*SD* = `r exp.table %>% filter(ID == "Expert") %>% pull("SD Exp.") %>% round(., digits = 1)`; range: `r exp.table %>%  filter(ID == "Expert") %>% pull("Min Exp.")`-`r exp.table %>%  filter(ID == "Expert") %>% pull("Max Exp.")`). 
`r demo.data %>% filter(ID == "Expert" & School_type == "Primary School") %>% pull("Percent_schooltype") %>% unique()`% of the experts were primary school teachers and `r demo.data %>% filter(ID == "Expert" & School_type == "Secondary School") %>% pull("Percent_schooltype") %>% unique()`% were secondary school teachers. `r exp.table %>% filter(ID == "Expert") %>% pull("Secondary Teaching Activities")`% of the experienced teachers were also engaged in an secondary teaching activity, such as lecturers at the university, main training supervisors for trainee teachers and subject advisers. 

The novice teachers (`r demo.data %>% filter(Gender == 1 & ID == "Novice") %>% pull(Gender) %>% length()` women; `r demo.table %>% filter(ID == "Novice") %>% pull("Gender female in percent")`%) had a mean age of `r demo.data %>% filter(ID == "Novice") %>% pull(Age) %>% mean() %>% round(., digits = 1)` years (*SD* = `r demo.table %>% filter(ID == "Novice") %>% pull("SD Age in years") %>% round(., digits = 1)`; range: `r demo.table %>%  filter(ID == "Novice") %>% pull("Min Age in years")`-`r demo.table %>%  filter(ID == "Novice") %>% pull("Max Age in years")`) with an average teaching experience of `r exp.table %>% filter(ID == "Novice") %>% pull("M Exp.") %>% mean() %>% round(., digits = 1)` years. On average, the student teachers were in their `r exp.table %>% filter(ID == "Novice") %>% pull("M Semester") %>% mean() %>% round(., digits = 1)` semester (*SD* = `r exp.table %>% filter(ID == "Novice") %>% pull("SD Semester") %>% round(., digits = 1)`; range: `r exp.table %>%  filter(ID == "Novice") %>% pull("Min Semester")`-`r exp.table %>%  filter(ID == "Novice") %>% pull("Max Semester")`). Furthermore, they had an average teaching experience of `r exp.table %>% filter(ID == "Novice") %>% pull("M Internship") %>% mean() %>% round(., digits = 1)` teaching units à 45min (*SD* = `r exp.table %>% filter(ID == "Novice") %>% pull("SD Internship") %>% round(., digits = 1)`; range: `r exp.table %>%  filter(ID == "Novice") %>% pull("Min Internship")`-`r exp.table %>%  filter(ID == "Novice") %>% pull("Max Internship")`) through the internships during their studies. 
`r demo.data %>% filter(ID == "Novice" & School_type == "Primary School") %>% pull("Percent_schooltype") %>% unique()`% of the novices were studying to become primary school teachers, `r demo.data %>% filter(ID == "Novice" & School_type == "Secondary School") %>% pull("Percent_schooltype") %>% unique()`% to become secondary school teachers and `r demo.data %>% filter(ID == "Novice" & School_type == "Special Education") %>% pull("Percent_schooltype") %>% unique()`% to become special education teachers. `r exp.table %>% filter(ID == "Novice") %>% pull("Extracurricular Teaching Experience")`% of the student teachers were also engaged in an extracurricular teaching activity, such as tutoring or homework supervision. 
