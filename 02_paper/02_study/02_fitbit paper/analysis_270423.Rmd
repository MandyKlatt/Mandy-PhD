...
for (i in unique(data_regression_mean$time_span)) {
  ...
  regr1 <-
    lm(heart_rate ~ disruption_factor,
    data = cache) 
  ...
  regr2 <-
    lm(heart_rate ~ confidence_factor,
    data = cache) 
  ...
  regr4 <-
    lm(heart_rate ~ disruption_factor + confidence_factor,
    data = cache)
...
ggsave(filename = "plots_publication/ki_plot.png",
       width = 16,
       height = 12,
       units = "cm")
```
